...
Если у вас уже есть локальный репозитории то первые четыре шага с предедущего раздела пропускаем и делаем следующие шаги:
1. Переименовываем назмание ветки что бы и в локальной и в удаленном репозитории они были одинаковы:
```
git branch -M main
```
2. Соединяем локальный и удалёный репозиторий:
```
git remote add origin https://github.com/(название вашего аккаунта)/(название вашего репозитори).git
```
3. Отправляем все изменения которые мы сделали в локальном репозитории в удалёный:
```
git push -u origin main
```
...
После чего откроется следующее окно где мы сможем изменить название репозиторяи или же оставить как есть, можем добавить какоето описание и нажимаем на Creat fork:
